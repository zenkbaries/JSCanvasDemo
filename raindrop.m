...
- (void)animateOneFrame
{
	...
	NSSize size;
	NSColor *color;
    NSColor *black_screen;
	float red, green, blue, alpha;
	int shapeType;
	ScreenSaverDefaults *defaults;
    float ran_width;
    NSRect full_screen;
    NSBezierPath *path_fade;

	size = [self bounds].size;
    ran_width = SSRandomFloatBetween( size.width / 100.0, size.width / 10.0 );

    full_screen.size = NSMakeSize(size.width, size.height);

	// Calculate random width and height
	rect.size = NSMakeSize( ran_width, ran_width);
...
//	alpha = SSRandomFloatBetween( 0.0, 255.0 ) / 255.0;
	...
	black_screen = [NSColor colorWithCalibratedRed:0 green:0 blue:0 alpha:.005];
	[color set];

	// And finally draw it
	defaults = [ScreenSaverDefaults defaultsForModuleWithName:MyModuleName];
    [black_screen set];
    path_fade = [NSBezierPath bezierPathWithRect:full_screen];
    [path_fade fill];
